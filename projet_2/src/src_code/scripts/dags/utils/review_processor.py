...
class ReviewProcessor:
    ...
    def _init_s3(self):
        ...
        self.s3_client = self.s3_hook.get_conn()  # S3 client is now reachable
        if not self.s3_client:
            logger.error("Failed to initialize S3 client.")
            raise ValueError("S3 client initialization failed.")
        # Alternative:
        self.s3_client = S3Hook(aws_conn_id='aws_s3_default')  # Use `get_conn()` to get S3 client
        if not self.s3_client:
            logger.error("Failed to initialize S3 client.")
            raise ValueError("S3 client initialization failed.")
    ...
    def load_table_from_s3(self, s3_uri: str) -> pd.DataFrame:
        ...
        logger.info("DEBUG ---> AWS_ACCESS_KEY_ID:" + str(aws_access_key_id))
        aws_secret_access_key = Variable.get("AWS_SECRET_ACCESS_KEY", default_var=None)
        logger.info("DEBUG ---> AWS_SECRET_ACCESS_KEY:" + str(aws_secret_access_key))
        region_name = Variable.get("AWS_REGION", default_var="eu-west-1")
        logger.info("DEBUG ---> AWS_REGION:" + str(region_name))
    ...
    def _init_snowflake(self):
        ...
        logger.info("DEBUG -> Schema raw:" + str(conn.extra_dejson.get("schema")))
        schema_raw = conn.extra_dejson.get("schema") or conn.schema or "" # "DB_AMZ/REVIEW"
        database, schema = schema_raw.split("/", 1)  
        logger.info("DEBUG -> Parsed database:" + str(database))
        logger.info("DEBUG -> Parsed schema:" + str(schema))
        ...
        self.snowflake_conn = snowflake.connector.connect(
            ...
            #conn.extra_dejson.get("warehouse"),
            role=conn.extra_dejson.get("role"),
        )
        logger.info("DEBUG -> conn.login:" + str(conn.login))
        logger.info("DEBUG -> conn.account:" + str(conn.extra_dejson.get("account") or conn.host))
        logger.info("DEBUG -> database:" + str(conn.extra_dejson.get("database")))
        logger.info("DEBUG -> schema:" + str(conn.extra_dejson.get("schema")))
        logger.info("DEBUG -> warehouse:" + str(conn.extra_dejson.get("warehouse")))
        logger.info("DEBUG -> role:" + str(conn.extra_dejson.get("role")))  
